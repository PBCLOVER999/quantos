...
def _rank_cross_section_for_day(df: pd.DataFrame) -> pd.DataFrame:
    """
    For a single date:
      - compute alpha_score = mom_252 / vol_20 (if available)
      - rank cross-sectionally
      - assign long / short signals
    """
    df = df.copy()
    df["raw_signal"] = 0.0
    df["cs_rank"] = np.nan
    ...
    # Require momentum at minimum
    tradable = df.dropna(subset=["mom_252"]).copy()

    if len(tradable) < MIN_ASSETS:
        return df

    # Vol-adjusted alpha if vol exists
    if VOL_COL in tradable.columns:
        vol = tradable[VOL_COL].replace(0, np.nan).clip(lower=MIN_VOL)
...
# ====================================================
# 3) MAIN ENTRYPOINT (PURE ALPHA)
# ====================================================

def compute_cross_sectional_momentum(df: pd.DataFrame) -> pd.DataFrame:
    """
    Cross-sectional momentum (vol-adjusted if vol_20 exists).
